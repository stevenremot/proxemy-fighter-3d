...
export class AiVessel extends WorldObject {
  ...
  createFsm() {
    ...
    this._fsm.addState("Aim").addCallback(() => (this._speed *= 2));
  ...
  computeSphericalVelocity() {
    ...
    let thetaSign = 1;
    if (tmpSphericalVector.theta > this._sphericalTarget.theta) thetaSign = -1;
    let phiSign = 1;
    if (tmpSphericalVector.phi > this._sphericalTarget.phi) phiSign = -1;
    let deltaTheta = Math.abs(
      tmpSphericalVector.theta - this._sphericalTarget.theta
    );
    let deltaPhi = Math.abs(tmpSphericalVector.phi - this._sphericalTarget.phi);

    this._sphericalVelocity
      .set(thetaSign * deltaTheta, phiSign * deltaPhi)
      .normalize();
  }
  ...
  update(dt) {
    ...
    if (
      this._fsm.currentState == "Detect" ||
      this._detectionCount >= DETECTION_FREQUENCY
    ) {
      if (this._fsm.currentState != "Spherical") this.performDetection();
      this._detectionCount = 0;
      ...
      if (this.hasReachedSphericalTarget()) this._fsm.setState("Chase");
    } else {
      ...
      if (this._fsm.currentState == "Chase") {
        ...
        if (tmpDirection.dot(this.target.up) < 0) this.up.multiplyScalar(-1);

        if (this._steerings.followIntensity < 0.3) this._fsm.setState("Aim");
      }
