...
export class Boss extends WorldObject {
  constructor(world, radius) {
    ...
    this.collisionBody = new Sphere(this.model.position.clone(), radius);
  ...
  onCollisionWith(object) {
    ...
    for (let bossModule of this.modules) {
      if (bossModule.isAlive() && bossModule.isPointInAngularRange(position)) {
  ...
  addModule(thetaRange, phiRange, c) {
    ...
    this.modules.push(
      this.world.createObject(Module, {
