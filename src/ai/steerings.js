...
export class Steerings {
  constructor(object, detector) {
    ...
    this._avoidance.set("avoidBoss", {
      vector: new THREE.Vector3(),
      update: () => this.avoidBoss()
    });
    this._avoidance.set("avoidVessels", {
      vector: new THREE.Vector3(),
      update: () => this.avoidVessels()
    });
  }
  ...
  follow() {
    ...
    tmpTargetPos
      .copy(this._followTarget)
      .add(this._target.forward.clone().multiplyScalar(FOLLOW_MIN));

    let distance = this._object.position.distanceTo(tmpTargetPos);
    let intensity = 1;
    if (distance < FOLLOW_MAX) intensity = distance / FOLLOW_MAX;
  ...
  stayVisible() {
    if (this._detector.isVisible(this._object))
      ...
      // go for it
      this._behaviour.get("stayVisible").vector.copy(orthoPoint.normalize());
    }
  }
  ...
  avoidBoss() {
    ...
    else if (dist >= AVOIDANCE_MAX) intensity = 0;

    this._avoidance
      .get("avoidBoss")
      .vector.copy(this._object.position)
      .normalize()
      .multiplyScalar(intensity);
  }
  ...
  avoidVessels() {
    ...
    for (let vessel of vessels) {
      if (vessel != this._object) {
        ...
        if (dist > AVOIDANCE_MIN && dist < AVOIDANCE_MAX)
          localIntensity =
            (AVOIDANCE_MAX - dist) / (AVOIDANCE_MAX - AVOIDANCE_MIN);
        else if (dist >= AVOIDANCE_MAX) localIntensity = 0;

        tmpVector
          .copy(this._object.position)
          .sub(vessel.position)
          .multiplyScalar(localIntensity);
        tmpTargetPos.add(tmpVector);
        if (intensity < localIntensity) intensity = localIntensity;
      }
    }
  ...
  computeDesiredVelocity() {
    ...
    if (tmpVelocity.lengthSq() > 1) tmpVelocity.normalize();
